...
class PowerUp {
  ...
  get color()  { return this.type === 'escudo' ? '#5d5' : '#0df'; }
  get accent() { return this.type === 'escudo' ? '#afa' : '#aff'; }
  get glow()   { return this.type === 'escudo' ? 'rgba(120, 255, 160, ' : 'rgba(0, 220, 255, '; }
  ...
  draw() {
    ...
    // Icono: rayo (Velocidad) o escudo (arco concéntrico)
    ctx.rotate(-this.rot);
...
function update(dt) {
  ...
  for (const p of powerups) {
    if (!p.dead && dist(ship, p) < ship.radius + p.radius) {
      p.dead = true;
      if (p.type === 'escudo') ship.shield     = SHIELD_DURATION;
...
function drawHUD() {
  ...
  // Indicador del power-up "Escudo" activo
  if (ship.shield > 0) {
    const BAR_X = 14, BAR_Y = 72, BAR_W = 120, BAR_H = 6;
    const pct = ship.shield / SHIELD_DURATION;

    ctx.fillStyle = '#5d5';
    ctx.textAlign = 'left';
    ctx.font      = '15px monospace';
    ctx.fillText('ESCUDO', BAR_X, BAR_Y - 4);
